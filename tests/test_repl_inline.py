...
class TestReplInlineImport:
    ...
    def test_import_repl_inline(self):
        ...
        from py2max.repl_inline import (
    ...
    def test_import_all_exports(self):
        ...
        from py2max import repl_inline

        assert "start_inline_repl" in repl_inline.__all__
        assert "start_background_server_repl" in repl_inline.__all__
        assert "BackgroundServerREPL" in repl_inline.__all__
        assert "setup_file_logging" in repl_inline.__all__
        assert "restore_console_logging" in repl_inline.__all__
...
class TestSetupFileLogging:
    ...
    def test_setup_file_logging_creates_file(self, tmp_path):
        ...
        from py2max.repl_inline import restore_console_logging, setup_file_logging
...
class TestStartInlineRepl:
    ...
    @pytest.mark.asyncio
    async def test_start_inline_repl_creates_log_dir(self, patcher, mock_server, tmp_path):
        """Test that start_inline_repl creates log directory."""
        from py2max.repl_inline import start_inline_repl

        log_file = tmp_path / "logs" / "server.log"

        # Mock start_repl (imported inside the function from py2max.repl)
        with patch("py2max.repl.start_repl", new_callable=AsyncMock):
    ...
    @pytest.mark.asyncio
    async def test_start_inline_repl_default_log_file(self, patcher, mock_server, tmp_path):
    ...
    @pytest.mark.asyncio
    async def test_start_inline_repl_restores_logging(self, patcher, mock_server, tmp_path):
...
class TestStartBackgroundServerRepl:
    ...
    async def test_start_background_server_repl_creates_repl(self, patcher, tmp_path):
        ...
        with patch(
            "py2max.repl_inline.BackgroundServerREPL.start", new_callable=AsyncMock
        ):
            # Should not raise
