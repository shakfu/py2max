...
- **Three patches, all generated by py2max**, one per thing worth showing. `v8-harness.maxpat` exercises every message. `save-demo.maxpat` covers the exact route out -- `save` as a message, `writePatch` from your own script. `serialize-demo.maxpat` covers the lossy one, and contains a small instrument placed by hand rather than built by script, because capturing what someone edited is the case `serialize` is actually for.
