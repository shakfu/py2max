...
### New: `ts/` -- experimental TypeScript core spike (built, evaluated, closed)

- Added `ts/`: the `.maxpat` format as TypeScript types plus a minimal `Patcher`/`Box`/`Patchline` model with JSON round-trip (868 lines, 32 passing tests, `make ts-check`; needs [Bun](https://bun.sh)). Layout, maxref, the database, the CLI and SVG export were out of scope by design. `ts/README.md` is the write-up.
