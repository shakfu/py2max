...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="maxfile-converter", description="converts max file to and from yml"
    )
    arg = option = parser.add_argument
    arg("target", help="target max file or yml file to convert")
    option(
        "--to-rnbopat",
        help="convert yaml to .rnbopat max file type",
        action="store_true",
    )
    option(
        "--to-maxhelp",
        help="convert yaml to .maxhelp max file type",
        action="store_true",
    )
